...
county.panel <- merge(county.panel, DT.hp.ret.2008Q34, by = "fips.code") %>%
  ##get the house price return and bartik interaction
  .[time %between% as.Date(c("2008-07-01", "2008-12-01")),
    bartik.qcew.2008Q34 := sum(bartik.qcew), by = fips.code] %>%
  .[, bartik.qcew.2008Q34 := mean(bartik.qcew.2008Q34, na.rm = TRUE), by = fips.code] %>%
  .[, hp.ret.bartik.qcew.2008Q34 := bartik.qcew.2008Q34 * zillow.ret.2008Q34]
...
control.vars <- c("zillow.ret.2008Q12", "zillow.ret.2008Q34", "hp.ret.bartik.qcew.2008Q34", "land.unavailable",
...
##The formulas
f.base.formula <- f_formula(c(base.vars, "hp.ret.bartik.qcew.2008Q34", "zillow.ret.2008Q34", "bartik.qcew"))
...
out <- data.table(type = c("Only 2008Q3/4 Bartik x HP Growth", "Full Model with All Controls"),
...
DT.for.graphs <- out[, tidy.ddd] %>% setNames(out$type) %>%
  ...
  .[, type := factor(type, levels = c("Only 2008Q3/4 Bartik x HP Growth",
...
saveRDS(out, "RdsFiles/36-county_reg_bartik_hp_ret2008Q34_control_base_full_all.rds")
saveRDS(DT.for.graphs,
        "RdsFiles/36-county_reg_bartik_hp_ret2008Q34_control_base_full_all_for_graphs.rds")
